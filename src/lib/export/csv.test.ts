...
describe("buildLibraryCsv", () => {
  it("genera header y escapa campos con comas", async () => {
    ...
    expect(lines[0]).toBe(
      "shelfside_id,title,media_type,source,external_id,status,score,current_season,last_episode_watched,started_at,completed_at,notes,image_url,catalog_updated_at,library_updated_at",
    );
